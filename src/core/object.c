...
void obj_render(const struct Object* _o) {
	Matrix4x4 t = M4x4_POS(_o->transform.translation.X, _o->transform.translation.Y, _o->transform.translation.Z);
	Matrix4x4 r = QUAT_TO_MAT4(_o->transform.rotation);
	
	bgfx_set_transform(MUL(t, r).Elements, 1);
...
void obj_encoder_render(bgfx_encoder_t* _encoder, const struct Object* _o) {
	...
	bgfx_encoder_set_transform(_encoder, MUL(t, r).Elements, 1);
