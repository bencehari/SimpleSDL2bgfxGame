...
void tr_rot_x(struct Transform* _t, float _angle) {
	_t->rotation = Q_MUL(_t->rotation, QUAT_AA(AXIS_X, _angle));
}

void tr_rot_y(struct Transform* _t, float _angle) {
	_t->rotation = Q_MUL(_t->rotation, QUAT_AA(AXIS_Y, _angle));
}

void tr_rot_z(struct Transform* _t, float _angle) {
	_t->rotation = Q_MUL(_t->rotation, QUAT_AA(AXIS_Z, _angle));
}
...
void tr_rot_xy(struct Transform* _t, float _xAngle, float _yAngle) {
	...
	else _t->rotation = Q_MUL(_t->rotation, Q_MUL(QUAT_AA(AXIS_X, _xAngle), QUAT_AA(AXIS_Y, _yAngle)));
}
